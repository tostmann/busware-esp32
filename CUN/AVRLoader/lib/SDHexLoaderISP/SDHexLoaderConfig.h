...
{
    // LEDs
    const uint8_t	kErrorPin		= 6;
    const uint8_t	kHeartbeatPin		= 7;
    const uint8_t	kProgModePin		= 4;

    // ISP RESET PIN
    const uint8_t	kResetPin		= 2;


    // OE not needed for Levelshifters
    const uint8_t	kReset3v3OEPin		= 3;
    const uint8_t	kISP_OE_Pin		= 3;

    const uint8_t	kSCK			= SCK;
