...
/********************************* FillBuffer *********************************/
void AVRStreamISP::FillBuffer(uint16_t inLength) {
...
void AVRStreamISP::WriteMemoryPage(
	...
	uint16_t	inAddress)
{
    Serial.print("WriteMemoryPage ");
    Serial.println(inAddress, HEX);
	//digitalWrite(Config::kProgModePin, LOW);
...
/********************************* WriteProgram *********************************/
void AVRStreamISP::WriteProgram(uint16_t inLength) {
    FillBuffer(inLength);
    if (read() == CRC_EOP)
	{
	    write(STK_INSYNC);
...
{
    Serial.println("AVRStreamISP::ReadSignature");
	if (read() == CRC_EOP)
	{
		write(STK_INSYNC);
