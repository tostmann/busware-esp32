	...
	uint32_t		inTimestamp)
{
	...
	*/
	...
	if (success)
	{
		...
		Serial.println("loading AVR config ...");
		
		success = avrConfig.SetDummy();
		...
		if (success)
		{
				...
				if (success)
				{
					...
					*/
					if (mConfig.bootloader)
					{
						...
						Uint16ToDecStr(mConfig.bootloader, &configPath[pathLen]);
						pathLen = strlen(configPath);
			...
			{
			    Serial.println("loading Hexfile ...");
				...
				if (success)
				{
					...
					*/
					...
					{
						mConfig.byteCount = IntelHexFile::EstimateLength();
					}
					
					Serial.print("FileSize: ");
					Serial.println(mConfig.byteCount);
							
					if (inAVRStreamISP)
					{
					    Serial.println("loading Hexfile ... ok");
						inAVRStreamISP->SetStream(&mContextualStream);
	...
	if (success)
	    {
		Serial.println("Begin Part II ... ok");
		mSyncRetries = 0;
		mError = 0;
...
void SDHexSession::SetDevice(
	bool	inIsResponse)
{
    Serial.println( "SDHexSession::SetDevice" );
    if (!inIsResponse)
	{
...
/******************************* ReadSignature ********************************/
void SDHexSession::ReadSignature(bool inIsResponse) {
    if (!inIsResponse) {
	WaitForAvailableForWrite(2);
	...
	for (uint8_t i = 0; i < 3; i++) {
	    uint8_t iss = mStream->read();
	    Serial.print(mConfig.signature[i], HEX);
	    Serial.println(iss, HEX);
	    if (mConfig.signature[i] == iss)
		{
		    continue;
		}
	...
	bool	inIsResponse)
{
	if (!inIsResponse)
	{
	    ...
	    Serial.println( "SDHexSession::ChipErase" );
	...
	bool	inIsResponse)
{
	if (!inIsResponse)
	{
	    ...
	    Serial.println( "SDHexSession::VerifyUnlocked" );
	...
	bool	inIsResponse)
{
	if (!inIsResponse)
	{
	    ...
	    Serial.println( "SDHexSession::VerifyFuse" );
